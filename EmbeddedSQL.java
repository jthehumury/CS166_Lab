...
public class EmbeddedSQL {
   ...
   public static void Query2(EmbeddedSQL esql){
      try{
         String query = "SELECT sid, COUNT(pid) FROM catalog GROUP BY sid HAVING COUNT(pid) >= 3";
   ...
   public static void Query3(EmbeddedSQL esql){
      try{
         String query = "SELECT sname, COUNT(pid) FROM suppliers NATURAL JOIN catalog WHERE sname IN\n";
   ...
   public static void Query4(EmbeddedSQL esql){
      try{
         String query = "SELECT sname, MAX(cost) FROM suppliers NATURAL JOIN catalog WHERE sname IN\n";
